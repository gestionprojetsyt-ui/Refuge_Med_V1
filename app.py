...
# --- 2. LA POP-UP (MULTI-AFFICHES) ---
@st.dialog("📢 ÉVÉNEMENTS AU REFUGE", width="large")
def afficher_evenement(liste_urls):
    # La plus récente (en bas de l'Excel) s'affiche en premier
    ...
    for i, url in enumerate(liste_ordonnee):
        if url:
            ...
            st.markdown(f"""<div style="text-align: center;"><img src="{url}" style="max-height: 65vh; max-width: 100%; border-radius: 10px; object-fit: contain;"></div>""", unsafe_allow_html=True)
            
            # Séparateur entre les affiches
            if i < len(liste_ordonnee) - 1:
                st.markdown("""<hr style="border: 0; border-top: 2px solid #eee; margin: 30px 0;">""", unsafe_allow_html=True)
...
try:
    ...
    if not df_config.empty:
        ...
        if not lignes_ev.empty and "popup_vue" not in st.session_state:
            liens_valides = [format_image_url(str(r.iloc[1])) for _, r in lignes_ev.iterrows() if str(r.iloc[1]) != "nan"]
            if liens_valides:
        ...
        for _, row in df_filtre.iterrows():
            with st.container(border=True):
                ...
                with col_img:
                    ...
                    st.image(url_photo if url_photo and url_photo.startswith('http') else "https://via.placeholder.com/300", use_container_width=True)
                with col_txt:
    ...
    st.markdown("""
        <div class="footer-container">
            ...
            <div style="font-size:0.85em; color:#666; margin-top:15px; padding-top:15px; border-top:1px solid #ddd;">
                ...
                <div class="version-note">Version b2.7 - Correctif Pop-up & Note de version</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
